...
if __name__ == "__main__":
    ...
    error_record = np.genfromtxt(f'pos_error_record_{2}.out',delimiter=',')[:,:]
    for i in range(number_files-2):
        #print(i+5)
        error_record_i = np.genfromtxt(f'pos_error_record_{i+3}.out',delimiter=',')[:,:]
    ...
    # load ref_mes s
    ref_mes = np.genfromtxt(f'ref_mes_{2}.out',delimiter=',')[:,:]
    for i in range(number_files-2):
        #print(i+5)
        ref_mes_i = np.genfromtxt(f'ref_mes_{i+3}.out',delimiter=',')[:,:]

...
class GPR:
    def __init__(self, optimize=True):
        ...
        self.params = {"l": 0.3, "sigma_f": 0.2,"alpha":1e-8}
    ...
    def fit(self, X, y,bounds_):
        ...
        if self.optimize:
            res = minimize(negative_log_likelihood_loss, [self.params["l"],self.params["alpha"]],
                   bounds=bounds_,
                   method='L-BFGS-B',options={'maxls':1200})
            self.params["l"],self.params["alpha"] = res.x[0],res.x[1]
        self.is_fit = True
...
def DT_gp_model(dT,name,predict,measurement,control,total_buff_size):
    ...
    #optimal_alpha_1 = 1e-8

    # find optimal L 
    gpr1 = GPR()
    gpr1.fit(input_state.T,error_y[[6],:].T , [(1e-2, 1e1),(1e-8, 1)] )
    #gpr1.fit(input_state.T,error_y[[6],:].T , [(1e-2, 1e1)],optimal_alpha_1)

    gpr2 = GPR()
    gpr2.fit(input_state.T,error_y[[7],:].T , [(1e-2, 1e1),(1e-8, 1)] )
    #gpr2.fit(input_state.T,error_y[[7],:].T , [(1e-2, 1e1)],optimal_alpha_1 )

    gpr3 = GPR()
    gpr3.fit(input_state.T,error_y[[2,8],:].T ,[(1e-2, 1e1),(1e-8, 1)] )
    #gpr3.fit(input_state.T,error_y[[2,8],:].T ,[(1e-2, 1e1)],optimal_alpha_1)

    # use optimal L train GP error model
    l_1 = gpr1.params['l']
    sig_f_1 = gpr1.params['sigma_f']
    optimal_alpha_1= gpr1.params['alpha']
    X = input_state
    Y = error_y[[6],:]
    ...
    optimal_alpha_2= gpr2.params['alpha']
    X = input_state
    Y = error_y[[7],:]
    error_2 = fit_gp(sig_f_2,l_2,X,Y,x,u,optimal_alpha_2)

    l_3 = gpr3.params['l']
    sig_f_3 = gpr3.params['sigma_f']
    optimal_alpha_3= gpr3.params['alpha']
    X = input_state
    Y = error_y[[2,8],:]
    error_3 = fit_gp(sig_f_3,l_3,X,Y,x,u,optimal_alpha_3)
    ...
    print('optimal_alpha_1:',optimal_alpha_1 ,'optimal_alpha_2  ',optimal_alpha_2,'optimal_alpha_3  ' ,optimal_alpha_3)

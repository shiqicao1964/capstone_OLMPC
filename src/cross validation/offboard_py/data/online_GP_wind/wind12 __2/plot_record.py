...
if __name__ == "__main__":
    ...
    error_record = np.concatenate((error_record_4,error_record_5,error_record_6), axis = 1)
    ...
    print('max error offline GP :', sorted(np.sqrt(np.sum(error_record_dyn,0)) , reverse=True)[:5]  )

    print('mean square error of online GP : (x , y , z)', np.mean(error_record,1))
    print('average distance error of online GP :', np.mean(np.sqrt(np.sum(error_record,0))))
    print('max error online GP :', sorted(np.sqrt(np.sum(error_record,0)) , reverse=True)[:5])
    ...
    ref_mes = np.concatenate((pos_record_4,pos_record_5,pos_record_6), axis = 1)

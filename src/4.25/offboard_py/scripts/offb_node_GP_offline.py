...
from MPC import rotation_matrix
...
rospy.Subscriber("mavros/local_position/pose", PoseStamped, callback = position_cb)
rospy.Subscriber("mavros/global_position/raw/gps_vel", TwistStamped, callback = V_cb)
...
rospy.Subscriber("reference", Twist, callback = refv_cb)
rospy.Subscriber("reference2", Pose, callback = refp_cb)
...
v_average = 2 # 1.5 / 2 / 2.5 / 3 radius = 8
# set traj =================================================================================================================================
x_ref,y_ref,z_ref,t_ref,vx_ref,vy_ref,vz_ref,T_onecircle = ellipse_trag(speed = v_average,x_w = 4,y_w = 4,z_w = 0,H = 3,dT = discretization_dt ,sim_t = 80)
...
model_gp_offline = DT_gp_model(t_horizon/N,f'GP_{0}',predict,measurement,controls,buff_Length)

acados_solver = acados_settinngs(model_gp_offline,t_horizon = t_horizon,N=N)


solver_save.append(acados_solver)
solver_save.popleft()
model_save.append(model_gp_offline)
model_save.popleft()
...
def main_control():
    ...
    while(not rospy.is_shutdown()):
        ...
        R_world_to_body = rotation_matrix(yaw, pitch, roll)
        V_world = np.array([Vx_mean, Vy_mean, Vz_mean])
        V_body = np.dot(np.linalg.inv(R_world_to_body), V_world)
        ...
        if (k < 500 or k > 600):
            ...
            if target.shape[0] == N+1:
                ...
                [Xc, Yc, Zc] + [roll, pitch, yaw] + [V_body[0], V_body[1], V_body[2]] + [Wxc, Wyc, Wzc]
                )
                ...
                if wind ==0 :
                    #np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/PREpredict.out',predict,delimiter=',')
                    #np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/PREmeasurement.out',measurement,delimiter=',')
                    #np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/PREcontrols.out',controls,delimiter=',')
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_nowind/posrecord_dyn.out',np.array(pos_record).T,delimiter=',')
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_nowind/pos_error_record_dyn.out',np.array(pos_error_record).T,delimiter=',')
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_nowind/ref_mes_dyn.out',np.array(ref_mes).T,delimiter=',')
                if wind == 1:
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_wind/posrecord_dyn.out',np.array(pos_record).T,delimiter=',')
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_wind/pos_error_record_dyn.out',np.array(pos_error_record).T,delimiter=',')
                    np.savetxt('/home/shiqi/catkin_ws/src/offboard_py/data/offline_GP_wind/ref_mes_dyn.out',np.array(ref_mes).T,delimiter=',')
                print('==========================================================')
                print('===================finished ! =========================')
                print('==========================================================')
                k = 601

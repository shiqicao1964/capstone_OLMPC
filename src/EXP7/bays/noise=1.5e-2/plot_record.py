...
if __name__ == "__main__":
    ...
    ax1.plot(ref_mes[3],ref_mes[4],ref_mes[5],color='orange',label='Bayesian Optimization Method regularization')
    ax1.plot(ref_mes[0],ref_mes[1],ref_mes[2],color='red',label = 'Reference')
    ax1.set_ylim(-x_w,x_w)
    ax1.set_zlim(H-1,H+1)
    ax1.legend()
    ax1.set_title('Bayesian Optimization Method regularization, added noise=1.5e-2')
    ax1.set_xlabel('X [m]')
    ax1.set_ylabel('Y [m]')
    ax1.set_zlabel('Z [m]')

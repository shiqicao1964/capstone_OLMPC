...
def DT_gp_model(dT,name,predict,measurement,control,total_buff_size):
    ...
    W = pca.components_
    inputstate_hand = np.dot(input_state.T, W.T).T

    # find optimal L 
    gpr1 = GPR()
    gpr1c = GPR()
    gpr1.fit(inputstate_hand.T,error_y[[2,6,7,8],:].T , [(0.1, 1)] )
    gpr1c.fit(inputstate_hand.T,error_y[[2,6,7,8],:].T , [(0.1, 1)] )
    ...
    Y = error_y[[2,6,7,8],:]
    error_1 = fit_gp_pca(sig_f_1,l_1,inputstate_hand,Y,reduced_inputfeatures)

    error_1 = cs.vertcat(cs.MX([0,0]),error_1[0],cs.MX([0,0,0]),error_1[1:],cs.MX([0,0,0]))
    ...
    model.disc_dyn_expr = xf + error_1 
    return model

...
import math
import time
from MPC import acados_settinngs, linear_quad_model, run_solver,DT_gp_model, DT_linear_model, solve_DT_nextState, eight_trag, ellipse_trag, race_traj
...
def half_circle(R,speed,H,radst,radstop,Ox,Oy):
    dT = 0.05
    t_abl = []
    vx = []
    vy = []
    vz = []
    t = np.linspace(radst, radstop, num=10000)
    x,y,z = circle(R,R,H,t,Ox,Oy)
    x0,y0,z0 = circle(R,R,H,t[0],Ox,Oy)
    ...
    #########################################
    x,y,z = circle(R,R,H,t_new,Ox,Oy)
    vx = np.array(vx)
    vy = np.array(vy)
    vz = np.array(vz)
...
radius = 8
# set traj =================================================================================================================================
x_ref,y_ref,z_ref,t_ref,vx_ref,vy_ref,vz_ref,T_onecircle = ellipse_trag(speed = v_average,
                                                                        x_w = 4,y_w = 4,z_w = 0,H = 5,dT = discretization_dt ,
                                                                        sim_t = 30)
...
# =========set race traj==========================\
x_ref_race,y_ref_race,z_ref_race,t_ref_race,vx_ref_race,vy_ref_race,vz_ref_race = race_traj(speed = v_average,
                                                                        x_w = 4,y_w = 4,z_w = 0,H = 4,dT = discretization_dt ,
                                                                        sim_t = 25)
ref_race = np.zeros((x_ref_race.shape[0],12))
ref_race[::,0] = x_ref_race
ref_race[::,1] = y_ref_race
ref_race[::,2] = z_ref_race
ref_race[::,6] = vx_ref_race
ref_race[::,7] = vy_ref_race
ref_race[::,8] = vz_ref_race
# =================set return=========================================================================================================================

Rx,Ry,Rz,Rvx,Rvy,Rvz = half_circle(19.1,5,4,1.746,1.66-np.pi,9.9,29)
ref_R = np.zeros((Rx.shape[0],12))
ref_R[::,0] = Rx
ref_R[::,1] = Ry
ref_R[::,2] = Rz
ref_R[::,6] = Rvx
...
def main_control():
    ...
    while(not rospy.is_shutdown()):
        ...
        Vx_mean = odom.twist.twist.linear.x
        Vy_mean = odom.twist.twist.linear.y
        Vz_mean = odom.twist.twist.linear.z
        Wxc = odom.twist.twist.angular.x
        Wyc = odom.twist.twist.angular.y
        Wzc = odom.twist.twist.angular.z
        '''
        if (k < 400 or k > 500):
            ...
            if finished_train[0] == 0:
                ...
                target = ref_R[iindex:iindex+N + 1]
                last_refPoint = ref_R[iindex+1]
            elif return_mode == 0:
                target = ref_race[iindex:iindex+N + 1]
                last_refPoint = ref_race[iindex+1]
            ...
            if target.shape[0] == N+1 :
                ...
                if finished_race == 0:
                    ...
                    pose.pose.position.x = x_ref_race[0]
                    pose.pose.position.y = y_ref_race[0]
                    pose.pose.position.z = z_ref_race[0]
                    # new ind 
                    k = 100
                    iindex = 1
                    finished_race = 1
                    ...
                    pose.pose.position.x = x_ref_race[-1]
                    pose.pose.position.y = y_ref_race[-1]
                    pose.pose.position.z = z_ref_race[-1]
                    # set stop pos 
                    iindex = 1
                    return_mode = 1
                else: 
